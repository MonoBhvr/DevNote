...
test('loadContent builds the project/series/post tree and excludes drafts', async () => {
  ...
  try {
    ...
    assert.equal(site.assetsByProject.marknote.images['block-diagram'].path, 'assets/images/block-diagram.png')
    assert.equal(site.assetsByProject.marknote.images['block-diagram'].publicPath, '/DevNote/assets/projects/marknote/images/block-diagram.png')
    assert.equal(site.assetsByProject.marknote.images['block-diagram'].alt, undefined)
  } finally {
